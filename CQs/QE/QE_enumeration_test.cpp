#ifndef RANDOMORDERENUMERATION_QERANDOMENUMERATIONTEST_H_H
#define RANDOMORDERENUMERATION_QERANDOMENUMERATIONTEST_H_H
...
typedef RandomOrderEnumeration<QERandomAccessIndex, FisherYatesShuffler, QEAnswer> QERandomOrderEnumeration;
typedef long long ll;
...
int main() {
    ...
    // open output file
    ofstream out;
    out.open("TestsOutput/QE_report.txt", ios::app);
    //out << endl;
    //out << "sample ratio [%], build time [sec], sample time [sec]" << endl;

#ifdef PRINT
    ofstream log;
    log.open("/home/shaizeevi/QE.csv", ios::out);
    sample_ratios = {1.0};
    assert(!log.fail());
#endif

    auto t1 = chrono::steady_clock::now();
    QERandomOrderEnumeration enume;
...
#endif //RANDOMORDERENUMERATION_QERANDOMENUMERATIONTEST_H_H