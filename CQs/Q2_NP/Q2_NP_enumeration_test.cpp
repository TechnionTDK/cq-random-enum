#ifndef RANDOMORDERENUMERATION_Q2_NPRANDOMENUMERATIONTEST_H_H
#define RANDOMORDERENUMERATION_Q2_NPRANDOMENUMERATIONTEST_H_H
...
typedef RandomOrderEnumeration<Q2_NPRandomAccessIndex, FisherYatesShuffler, Q2_NPAnswer> Q2_NPRandomOrderEnumeration;
typedef long long ll;
...
int main() {
    ...
    // open output file
    ofstream out;
    out.open("TestsOutput/Q2_NP_report.txt", ios::app);
    //out << endl;
    //out << "sample ratio [%], build time [sec], sample time [sec]" << endl;

    #ifdef PRINT
    ofstream log;
    log.open("TestsOutput/Q2_NP_answers.csv", ios::out);
    sample_ratios = {1.0};
    #endif

    auto t1 = chrono::steady_clock::now();
    Q2_NPRandomOrderEnumeration enume;
...
#endif //RANDOMORDERENUMERATION_Q2_NPRANDOMENUMERATIONTEST_H_H