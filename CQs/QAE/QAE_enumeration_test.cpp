#ifndef RANDOMORDERENUMERATION_QAERANDOMENUMERATIONTEST_H_H
#define RANDOMORDERENUMERATION_QAERANDOMENUMERATIONTEST_H_H
...
typedef RandomOrderEnumeration<QAERandomAccessIndex, FisherYatesShuffler, QAEAnswer> QAERandomOrderEnumeration;
typedef long long ll;
...
int main() {
    ...
    // open output file
    ofstream out;
    out.open("TestsOutput/QAE_report.txt", ios::app);
    //out << endl;
    //out << "sample ratio [%], build time [sec], sample time [sec]" << endl;

    #ifdef PRINT
    ofstream log;
    log.open("TestsOutput/QAE_answers.csv", ios::out);
    sample_ratios = {1.0};
    #endif

    auto t1 = chrono::steady_clock::now();
    QAERandomOrderEnumeration enume;
...
#endif //RANDOMORDERENUMERATION_QAERANDOMENUMERATIONTEST_H_H