...
typedef RandomOrderEnumeration<QARandomAccessIndex, FisherYatesShuffler, QAAnswer> QARandomOrderEnumeration;
typedef long long ll;
...
int main() {
    ...
    // open output file
    ofstream out;
    out.open("TestsOutput/QA_report.txt", ios::app);
    //out << endl;
    //out << "sample ratio [%], build time [sec], sample time [sec]" << endl;

#ifdef PRINT
    ofstream log;
    log.open("/home/shaizeevi/QA.csv", ios::out);
    sample_ratios = {1.0};
    assert(!log.fail());
#endif

    auto t1 = chrono::steady_clock::now();
    QARandomOrderEnumeration enume;
